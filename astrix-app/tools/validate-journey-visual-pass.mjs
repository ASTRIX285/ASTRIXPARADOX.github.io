...
assert.ok(html.includes('href="./journey-2560-visual.css?v=20260920-director-2&amp;markers=20260925-24&amp;chestnote=20260925-25&amp;gloss=20260926-1"'),'Journey must load the contained emblem and compact stats without stale page CSS');
...
assert.match(guardianAuth,/if\(session\?\.authenticated===true\)\{[\s\S]*?control\.button\.hidden=true;[\s\S]*?control\.visual\.hidden=false;/,'Connected destinations must replace the text button with only the account visual');
...
assert.ok(mapModule.includes('class="journey-region-chest-unknown" data-region-chest-unknown'));
