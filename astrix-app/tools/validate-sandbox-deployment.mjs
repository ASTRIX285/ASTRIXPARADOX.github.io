...
assert.match(authWorker,/const CHARACTER_PAGE_PROFILE_COMPONENTS = \[[\s\S]*?305\s+\/\/ ItemSockets/,'Character first paint must request exact equipped sockets without the large reusable-plug alternative graph.');
assert.match(authWorker,/const CHARACTER_PAGE_PROFILE_COMPONENTS = \[[\s\S]*?204, \/\/ CharacterActivities \(required by live inventory mutation preflight\)/,'The lightweight Character/live-action profile must include component 204 so item transfers can pass verified activity preflight.');
assert.match(authWorker,/profileScope === "character"[\s\S]*?CHARACTER_PAGE_PROFILE_COMPONENTS[\s\S]*?profileScope === "forge"[\s\S]*?CHARACTER_PROFILE_COMPONENTS[\s\S]*?profileScope === "journey"[\s\S]*?JOURNEY_PROFILE_COMPONENTS/,'The auth Worker must select each lightweight page profile scope explicitly');
...
assert.match(authWorker,/requestedFreshness[\s\S]*?profileUrl\.searchParams\.set\("freshness", requestedFreshness\)/,'Prepared page refreshes must propagate live freshness to the Bungie profile route.');
