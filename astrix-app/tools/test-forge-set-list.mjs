...
import {forgeLoaderEvaluateReady} from '../pages/forge-loader/forge-loader-residency.mjs';
...
const stagedRealLoad=new Map(owned.map(item=>[item.slotIndex,item]));
assert.equal(forgeLoaderEvaluateReady({ready:true},stagedRealLoad,true),true,'Miguel\'s real Nothing Manacles and four-piece Smoke Jumper shape must unlock immediately when current residency is ready.');
assert.equal(forgeLoaderEvaluateReady({ready:false},stagedRealLoad,true),false,'The same real five-piece shape must stay locked while a source is genuinely incomplete.');
...
console.log('FORGE_LOADER_REAL_STAGED_HANDOFF=PASS Nothing Manacles and four Smoke Jumper slots');
