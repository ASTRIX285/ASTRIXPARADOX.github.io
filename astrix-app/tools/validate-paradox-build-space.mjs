...
assert.match(runtime,/function renderArmourRecommendationState\(build=\{\}\)[\s\S]*?staged=Boolean\(build\.forgeLoaderDecision\)[\s\S]*?ARMOUR & INSTALLED MODS[\s\S]*?if\(staged&&!generated&&!manual\)blankArmourModCanvas\(\)/,'Only untouched Forge Loader staging may hide installed mods behind pending recommendation slots; equipped, saved, manual and generated builds retain their mod display.');
