...
// Prompt 21: preserve capped art and 6px inter-slot gap while adding a 22px menu control plus 4px internal gap.
assert.match(sources.shared,/html body \.guardian-loadouts-strip \.guardian-loadouts-grid\{\s*grid-template-columns:repeat\(auto-fill,calc\(var\(--apx-equipment-icon-size,66px\) \+ 26px\)\)!important;[\s\S]*?gap:6px!important;justify-content:start!important;/,'Character strip reserves exactly 26px for the adjacent menu without stretching gear');
