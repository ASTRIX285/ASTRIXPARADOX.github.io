...
assert.match(html,/forge-loader\.css\?v=20260908-icon-hover-1/,'Forge Loader must refresh the completed icon-token wiring without stale page CSS.');
...
assert.match(css,/@media\(max-width:820px\)\{[\s\S]*?\.forge-matrix-row\{grid-template-columns:minmax\(0,1fr\) var\(--apx-icon-card\) 6\.8rem\}/,'Mobile Forge Matrix rows must preserve the shared card icon width.');
assert.match(css,/\.forge-set-head\{[^}]*grid-template-columns:var\(--apx-icon-set-head\)[\s\S]*?\.forge-set-head img\{width:var\(--apx-icon-set-head\);height:var\(--apx-icon-set-head\)/,'Set Protocol records must consume the exact shared set-head icon token.');
