...
assert.match(css,/\.forge-set-choice\.is-owned \.forge-set-trait-icon\{background:#367fa6/,'Owned feasible set traits must use the blue Bungie-style icon background.');
assert.match(css,/\.forge-set-icon,\.forge-set-trait-icon\{[^}]*background:#151318/,'Unowned set icons retain a dark background.');
...
assert.match(read('astrix-app/pages/guardian-workspace-v2/paradox-build-space/index.html'),/paradox-build-space\.mjs\?v=20260908-icon-hover-1/,'Build Forge must load the complete protected build transfer and hover wiring without stale browser code.');
...
assert.match(css,/\.forge-exotic\.is-selected\{[^}]*border:1px solid #d62d3a[^}]*box-shadow:inset 0 0 0 3px #d62d3a,inset 0 0 0 4px #ffd965[^}]*animation:forge-exotic-selected-pulse/,'The selected Exotic must keep a fixed one-pixel footprint and an unmistakable inset animated perimeter.');
assert.doesNotMatch(css,/\.forge-exotic\.is-selected\{[^}]*border:(?!1px)[^;}]+/,'Selecting an Exotic must not change its tile footprint.');
...
assert.match(css,/\.forge-matrix-stat-reading\{display:flex;align-items:center;justify-content:center;gap:\.1rem\}[\s\S]*?\.forge-matrix-stat-icon\{width:var\(--apx-icon-inline\);height:var\(--apx-icon-inline\)/,'Each compact Matrix square must align the real Bungie stat glyph directly beside its calculated value.');
assert.match(css,/@container\(max-width:38\.75rem\)[\s\S]*?\.forge-matrix-stats\{grid-column:1\/-1;grid-row:2\}/,'Forge Matrix rows must stay in one line at the real 630-pixel output width and reflow only below it.');
assert.match(css,/\.forge-matrix-row\{grid-template-columns:5rem 5rem max-content 2\.2rem 3\.4rem minmax\(4\.5rem,1fr\);gap:\.2rem;padding:\.3rem \.25rem\}/,'Desktop Forge Matrix rows must keep equal leading columns and place the compact stat strip directly after the Exotic.');
assert.match(css,/\.forge-matrix-stats\{grid-template-columns:repeat\(6,2\.55rem\);justify-content:start;gap:\.15rem\}[\s\S]*?\.forge-matrix-stats>\.forge-matrix-stat\{width:2\.55rem;height:2\.55rem;aspect-ratio:1;padding:\.12rem \.06rem;overflow:hidden\}/,'Forge Matrix stat cells must remain six equal compact squares in one row.');
...
assert.match(css,/@container\(max-width:38\.75rem\)\{\.forge-matrix-row\{grid-template-columns:5rem 5rem minmax\(4\.25rem,1fr\) 4\.2rem 5\.4rem[\s\S]*?\.forge-matrix-protocol\{grid-column:3;grid-row:1\}\.forge-matrix-total\{grid-column:4;grid-row:1\}/,'Narrow Forge Matrix rows must preserve equal leading columns and keep protocol ahead of Raw Total.');
assert.match(css,/@media\(max-width:820px\)\{[\s\S]*?\.forge-matrix-row\{grid-template-columns:minmax\(0,1fr\) 5rem 5\.4rem\}[\s\S]*?\.forge-matrix-stats\{[^}]*grid-template-columns:repeat\(3,2\.55rem\)/,'Mobile Forge Matrix rows must retain compact square stat cells.');
assert.match(css,/\.forge-matrix-protocol-icons\{gap:\.12rem\}/,'Multiple real set protocol icons must stack closely in one Matrix column.');
assert.match(css,/\.forge-matrix-protocol-icon\{width:var\(--apx-icon-set-bonus\);padding:0\}[\s\S]*?\.forge-matrix-protocol-icon \.forge-set-trait-icon\{width:var\(--apx-icon-set-bonus\);height:var\(--apx-icon-set-bonus\)\}/,'Matrix protocol circles must use the shared compact set bonus icon size without changing the Set Protocol panel.');
