...
assert.ok(selectorIndex>=0&&selectorIndex<directivesIndex&&directivesIndex<outputIndex,'Forge Loader desktop DOM must order selection, directives and Forge Matrix as three columns.');
assert.ok(html.indexOf('forge-stat-selector')<html.indexOf('forge-staged')&&html.indexOf('forge-staged')<outputIndex,'Working Load must use the freed middle-column space directly beneath Stat Directive.');
assert.ok(html.indexOf('class="forge-set-drawer"')>outputIndex&&html.indexOf('class="forge-set-drawer"')<html.indexOf('forge-set-selector'),'Set Protocol must live in its collapsible side panel outside the three-column workspace.');
...
assert.match(html,/forge-loader\.css\?v=20260911-set-drawer-1/,'Forge Loader must load the Set Bonus side-panel layout without stale page CSS.');
