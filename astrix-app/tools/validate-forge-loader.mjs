...
assert.match(html,/forge-loader\.css\?v=20260909-set-list-1/,'Forge Loader must refresh the completed icon-token wiring without stale page CSS.');
...
assert.match(css,/\.forge-matrix-row\{[^}]*grid-template-columns:6\.7rem var\(--apx-icon-gear-art-width\) minmax\(22rem,1fr\)/,'Desktop Forge Matrix rows must reserve the shared armour art width.');
assert.match(css,/\.forge-matrix-exotic\{[^}]*width:var\(--apx-icon-gear-art-width\);height:var\(--apx-icon-gear-art-height\);[^}]*justify-self:center;align-self:center/,'Every matched Exotic frame must consume the shared armour portrait tokens.');
assert.match(css,/@container\(max-width:60rem\)\{\.forge-matrix-row\{grid-template-columns:6\.7rem var\(--apx-icon-gear-art-width\) minmax\(0,1fr\)/,'Compact Forge Matrix rows must preserve the shared armour art width.');
assert.match(css,/@media\(max-width:820px\)\{[\s\S]*?\.forge-matrix-row\{grid-template-columns:minmax\(0,1fr\) var\(--apx-icon-gear-art-width\) 6\.8rem\}/,'Mobile Forge Matrix rows must preserve the shared armour art width.');
