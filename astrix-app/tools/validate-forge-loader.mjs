...
import {createBuildState,restoreWorkingBuild} from '../pages/guardian-workspace-v2/paradox-build-space/paradox-build-state.mjs';
...
const equippedArmour=[item(0,2100,'equipped-helmet',3),item(1,2101,'equipped-gauntlets',4),item(2,2102,'equipped-chest',5),item(3,2103,'equipped-legs',6),item(4,2104,'equipped-class-item',7)].map((piece,slot)=>({...piece,mods:Array.from({length:6},(_,socket)=>({hash:3100+slot*10+socket,name:`Equipped mod ${slot}:${socket}`,icon:`/mod-${slot}-${socket}.png`,socketIndex:socket,semanticRole:socket===0?'masterwork':socket<3?'general-mod':'slot-mod'}))}));
const equippedWeapons=Array.from({length:3},(_,slot)=>({itemHash:5100+slot,itemInstanceId:`equipped-weapon-${slot}`,name:`Equipped weapon ${slot}`,mods:[{hash:6100+slot,socketIndex:1,name:`Equipped perk ${slot}`,icon:`/perk-${slot}.png`}]}));
...
const equippedProfileBuild={...binding,source:'bungie-live',characterClass:'hunter',subclass:'void',subclassName:'Nightstalker',subclassItemInstanceId:equippedSubclass.itemInstanceId,subclassItem:equippedSubclass,subclassCatalog:[equippedSubclass],subclassBuild:{super:{hash:2722573682,name:'Spectral Blades',icon:'/equipped-super.png'}},weapons:equippedWeapons,armour:equippedArmour,stats:[],loadoutsAvailable:true,loadouts:equippedLoadouts};
...
assert.deepEqual(protectedApplied.state.originalBuild.armour.map(row=>row.mods),equippedArmour.map(row=>row.mods),'All installed armour sockets must survive in the original equipped snapshot after staging.');
assert.deepEqual(protectedApplied.state.originalBuild.weapons,equippedWeapons,'Original weapons and their installed perks must survive Forge Loader handoff and staging.');
assert.deepEqual(protectedApplied.state.workingBuild.weapons,equippedWeapons,'Armour staging must preserve equipped weapons until an explicit working change.');
assert.deepEqual(protectedApplied.state.originalBuild.subclassBuild,equippedProfileBuild.subclassBuild,'Armour staging must preserve the original equipped Super.');
const restoredEquipped=restoreWorkingBuild(protectedApplied.state);
assert.deepEqual(restoredEquipped.workingBuild,protectedSnapshot.originalBuild,'Restore Original must recover the complete equipped build, including all mods, weapons and Super.');
...
const recoverableStore=memoryStorage();recoverableStore.setItem('astrix:bungie-page-cache-fallback:v4:loadout','large-profile');recoverableStore.setItem('astrix:bungie-loadout-cache-fallback:v2:hunter-1:0','large-loadout');recoverableStore.setItem('astrix:bungie-session-cache:v1','keep-session');
...
const residency=read('astrix-app/pages/forge-loader/forge-loader-residency.mjs');
...
assert.match(preload,/prepareForgeLoaderEntry[\s\S]*?preloadForgeLoaderPayload\(session,\{force:false\}\)/,'Forge Loader entry must use the prepared display snapshot instead of blocking navigation on a live Bungie read.');
...
assert.match(backend,/await enrichPreparedPageAccount\(payload, env, page,[\s\S]*?const response = preparedPageEnvelope\(request, env, payload, prepared\)[\s\S]*?return response/,'Prepared page semantics must finish on the account prefix before the public page bundle starts streaming.');
...
assert.ok(html.indexOf('forge-stat-selector')<html.indexOf('forge-set-selector')&&html.indexOf('forge-set-selector')<outputIndex,'Set Protocol must sit inline underneath Stat Directive in the middle column.');
...
assert.match(read('astrix-app/pages/forge-loader/forge-loader-set-list.mjs'),/type="checkbox"/,'Set bonuses must use checkboxes, not toggle switches.');
assert.match(runtime,/setBonusOptions\(armourItems\(\),exotic,setSelections\)/);
assert.match(read('astrix-app/pages/forge-loader/forge-loader-set-list.mjs'),/forge-set-trait-icon[\s\S]*?effect\.icon/,'Set list renders Bungie trait icons.');
assert.match(read('astrix-app/pages/forge-loader/forge-loader-set-list.mjs'),/perkTooltipAttributes[\s\S]*?effect\?\.description/,'Set descriptions use the shared tooltip.');
...
assert.match(runtime,/fetch\(new URL\('\/bungie\/forge\/armour-combinations',AUTH_ORIGIN\)[\s\S]*?'X-CSRF-Token':text\(session\?\.csrfToken\)/,'Forge Loader must delegate exhaustive calculation to the authenticated backend Worker.');
assert.doesNotMatch(runtime,/matchTopArmourBuilds/,'Forge Loader must not perform the exhaustive armour scan on the browser main thread.');
assert.match(runtime,/openProtocolSolverEvidence\(exotic,sourceItems\)/,'The backend scan must receive verified Exotic-to-set tie-break evidence before retaining its top 50 results.');
assert.match(backend,/armourCombinationsRoute[\s\S]*?verifySessionCharacter[\s\S]*?currentPreparedManifestVersion[\s\S]*?manifest_version_changed[\s\S]*?solveArmourCombinations/,'The backend solver route must enforce the session Guardian and current live manifest version before calculation.');
assert.match(backendSolver,/const visit = \(slot: number, exoticCount: number\)[\s\S]*?combinationsEvaluated \+= 1[\s\S]*?heap\.sort\(compareCandidates\)/,'The backend Worker must scan all legal combinations while retaining only its bounded best candidates.');
assert.match(backendSolver,/targetMaximums\[key\] = Math\.min\(STAT_CAP[\s\S]*?targetMaximums, completeScan: true/,'The backend scan must calculate exact achievable stat ceilings with the same live inventory pass.');
assert.match(runtime,/result\.targetMaximums\?\.\[key\]/,'The browser must consume Worker-calculated stat ceilings instead of recomputing combinations locally.');
...
const exoticSelectorRuntime=runtime.slice(runtime.indexOf('function renderExotics()'),runtime.indexOf('function bonusReason'));
assert.match(exoticSelectorRuntime,/<img src="\$\{esc\(group\.icon\)\}"/,'The Exotic selector must render only the Bungie definition image.');
assert.doesNotMatch(exoticSelectorRuntime,/itemTileMarkup|tileItem|power/,'The Exotic selector must not render power, lock, socket, watermark or item-card overlays.');
...
assert.match(read('astrix-app/pages/guardian-workspace-v2/paradox-build-space/index.html'),/paradox-build-space\.mjs\?v=20260913-character-safe-2/,'Build Forge must load the partial-data-safe protected build transfer without stale browser code.');
...
assert.match(buildRuntime,/async function initialiseBuildForge\(\)[\s\S]*?await restoreAtomicForgeTransfer\(\)[\s\S]*?atomicTransfer\|\|readState\(\)[\s\S]*?finally\{\s*initialisingBuild=false;\s*if\(pendingEquippedContext\)\{const detail=pendingEquippedContext;pendingEquippedContext=null;recoverMissingBuild\(detail\);\}\s*render\(\);[\s\S]*?queueMicrotask\(\(\)=>void refreshForgeArtifactRecommendation\(\)\);\s*\}/,'The initial Forge Loader handoff must restore its atomic record before releasing queued equipped context, rendering and starting asynchronous preparation work.');
assert.doesNotMatch(runtime,/\/bungie\/actions\/|EquipItems?|TransferItem|PullFromPostmaster|SetItemLockState|socket-plug-free/,'Forge Loader must never call a live Bungie mutation route.');
