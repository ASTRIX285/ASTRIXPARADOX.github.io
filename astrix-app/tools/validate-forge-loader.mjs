...
assert.match(buildRuntime,/async function initialiseBuildForge\(\)[\s\S]*?await restoreAtomicForgeTransfer\(\)[\s\S]*?atomicTransfer\|\|readState\(\)[\s\S]*?finally\{\s*render\(\);[\s\S]*?queueMicrotask\(\(\)=>void refreshForgeArtifactRecommendation\(\)\);\s*\}/,'The initial Forge Loader handoff must restore its atomic record and release the 58% gate before asynchronous preparation work.');
