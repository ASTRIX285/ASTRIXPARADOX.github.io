...
assert.match(html,/forge-loader\.mjs\?v=20260909-matrix-layout-1/,'Forge Loader must load the Matrix layout structure without stale browser code.');
assert.match(html,/forge-loader\.css\?v=20260909-matrix-layout-1/,'Forge Loader must load the responsive set and Matrix layout without stale page CSS.');
...
assert.match(read('astrix-app/pages/forge-loader/forge-loader-set-list.mjs'),/longestName=Math\.max[\s\S]*?class="forge-set-grid" style="--forge-set-name-width:\$\{longestName\}ch"/,'Set card width must be derived from the longest verified set name.');
assert.match(css,/\.forge-set-grid\{[^}]*grid-template-columns:repeat\(4,calc\(var\(--apx-icon-record\) \+ \.7rem \+ var\(--forge-set-name-width\) \+ 1\.5rem\)\)/,'Wide Set Protocol lists must use four equal columns sized from the longest verified set name.');
assert.match(css,/@container\(max-width:66rem\)[\s\S]*?repeat\(3,[\s\S]*?@container\(max-width:50rem\)[\s\S]*?repeat\(2,[\s\S]*?@container\(max-width:34rem\)[\s\S]*?grid-template-columns:calc/,'Set Protocol columns must wrap from four to three, two and one as the panel narrows.');
