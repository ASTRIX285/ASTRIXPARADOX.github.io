...
assert.match(html,/forge-loader\.css\?v=20260909-matrix-single-row-1/,'Forge Loader must load the single-row Matrix layout without stale page CSS.');
...
assert.match(css,/\.forge-matrix-row\{grid-template-columns:5rem 5rem max-content 3\.3rem 3\.4rem minmax\(4\.5rem,1fr\);gap:\.2rem;padding:\.3rem \.25rem\}/,'Desktop Forge Matrix rows must keep equal leading columns and place the compact stat strip directly after the Exotic.');
