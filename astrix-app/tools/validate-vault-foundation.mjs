...
assert.match(vaultHtml,/id="vaultActionDialog"[\s\S]*?CONFIRM LIVE ACTION/,'Direct equip may retain its explicit final confirmation surface.');
...
assert.match(vaultRuntime,/function stageTransfer\(item,destination\)[\s\S]*?pendingVaultAction=\{kind:'transfer',intent\}[\s\S]*?void performPendingVaultAction\(\)/,'Selecting MOVE must execute the exact live transfer immediately without a second confirmation click.');
...
assert.match(characterRuntime,/function stageCharacterVaultTransfer\(requestedItemKey\)[\s\S]*?pendingAction=\{kind:'transfer',intent\}[\s\S]*?void performCharacterInventoryAction\(\)/,'Character-to-Vault MOVE must execute immediately without a second confirmation click.');
assert.doesNotMatch(characterRuntime,/function stageCharacterVaultTransfer\(requestedItemKey\)\{[\s\S]*?showCharacterInventoryAction\('Confirm live Vault transfer'/,'Character movement must not open a redundant confirmation dialog.');
