...
assert.match(vaultRuntime,/executeVaultTransferIntent\(confirmVaultTransferIntent\(action\.intent\)[\s\S]*?if\(result\.attemptCount>0\|\|result\.mutationCount>0\|\|result\.readback\?\.verified\)await refreshAfterLiveAction\(\)/,'The UI must call the confirmed live executor and refresh after mutations or an idempotent live verification.');
