...
assert.match(liveActionsRuntime,/projectAcceptedInventoryLocation[\s\S]*?await onAccepted\(/,'A successful Bungie response must publish the exact accepted item location immediately while fresh verification continues.');
// Prompt 19: retain the accepted-item/refresh ordering and check its new status.
assert.match(vaultRuntime,/const onAccepted=async\(\{liveInventory\}\)=>\{await refreshAfterLiveAction\(liveInventory\)[\s\S]*?Item moved\. Updating inventory…/,'Vault must render Bungie-accepted item movement without blocking on delayed profile propagation.');
