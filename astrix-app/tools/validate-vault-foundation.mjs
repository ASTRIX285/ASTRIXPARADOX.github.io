...
assert.match(vaultRuntime,/const onAccepted=async\(\{liveInventory\}\)=>\{await refreshAfterLiveAction\(liveInventory\)[\s\S]*?final inventory verification continues/,'Vault must render Bungie-accepted item movement without blocking on delayed profile propagation.');
