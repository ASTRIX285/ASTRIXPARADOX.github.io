...
assert.match(vaultRuntime,/executeVaultTransferIntent\(confirmVaultTransferIntent\(action\.intent\)[\s\S]*?if\(result\.attemptCount>0\|\|result\.mutationCount>0\)await refreshAfterLiveAction\(\)/,'The UI must call the confirmed live executor and refresh after every Bungie mutation attempt.');
