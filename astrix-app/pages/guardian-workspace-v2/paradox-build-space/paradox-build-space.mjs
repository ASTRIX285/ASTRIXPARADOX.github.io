...
import {ForgePreparationClient,preparationVariants,forgePreparationKey} from './paradox-forge-preparation.mjs?v=20260916-weapon-combinations-2&entry=20260921-direct-1&plain=20260925-1';
...
import {armBuildTest,collectBuildTestResults,confirmCandidateActivity,captureMatchesCharacter,readCapture,readCaptureArchive} from '../guardian-shooting-range-capture.mjs?v=20260902-shared-account-orbit-1&plain=20260925-1';
import {analyzeLiveGuardian,renderLiveAnalysis} from '../guardian-paradox-live-adapter.mjs?v=20260905-background-forge-1&plain=20260925-1';
import {createLiveTransferPlan} from '../guardian-perk-change-plan.mjs?v=20260920-empty-sockets-1&plain=20260925-1';
import {liveActionCapabilities,stageLiveTransferPreflight,confirmLiveTransferPlan,executeLiveTransferPlan} from '../guardian-live-actions.mjs?v=20260906-live-equip-1&roll=20260909-apply-1&review=20260911-confirmation-1&copy=20260922-readable-block-1&plain=20260925-1';
import {armourCard} from '../guardian-gear-layout.mjs?v=20260908-set-icons-1&weapons=20260909-presentation-1&roll=20260909-apply-1&fix=20260909-apply-refresh-1&champion=20260924-champion-export-1&plain=20260925-1';
import {renderWeapons,weaponPerkMatrixMarkup,weaponTraitHierarchyMarkup} from '../guardian-semantic-ui.mjs?v=20260913-breaker-icon-2&ui=20260917-no-diagnostics-1&status=20260917-compact-1&champion=20260924-champion-export-1&plain=20260925-1';
...
import {markGuardianFastReturn,readForgeLoaderTransfer,cacheBuildForgeState,readBuildForgeState} from '../guardian-session-cache.mjs?v=20260913-live-character-2&plain=20260925-1';
import {guardianManifest} from '../guardian-manifest-service.mjs?v=20260906-all-page-data-1&roll=20260909-apply-1&champion=20260924-champion-export-1&plain=20260925-1';
...
import {applyForgeArtifactRecommendation,artifactPerkCatalogue} from './paradox-artifact-selection.mjs?v=20260916-unique-artifact-picks-1&plain=20260925-1';
import {BUILD_ELEMENTS,directEntryMode,directArmourChoices,withDirectGenerationContext,createDirectGenerationBuild,validateForgeGenerationEntry} from './paradox-build-recommendation.mjs?v=20260921-direct-1&plain=20260925-1';
import {composeForgeRecommendation,filterExoticCompatibleSubclasses,hasVerifiedSubclassSockets,rankExoticSuperSynergy,synchroniseSubclassProjection} from './paradox-forge-intelligence.mjs?v=20260909-super-evidence-1&plain=20260925-1';
...
import {eligibleEquipment,filterManualEquipmentSources,recordManualEdit,socketGroups,stageEquipmentChoice,stageSocketChoice,stageSubclassSocketChoice} from './paradox-manual-editor.mjs?v=20260910-tier-zero-evidence-1&plain=20260925-1';
import {saveParadoxLoadout} from './paradox-saved-loadouts.mjs?v=20260919-account-sync-1&plain=20260925-1';
...
import {reportPreparedPageStage} from '../../../core/prepared-page-client.mjs?v=20260913-workspace-preload-1&transport=20260911-compact-plugs-1&navigation=20260919-1&plain=20260925-1';
import '../guardian-character-cards.mjs?v=20260824-bungie-icons-3&loader=2';
import '../guardian-loadouts.mjs?v=20260905-loadout-actions-1&plain=20260925-1';
import {normaliseLiveProfile} from '../guardian-bungie-profile.mjs?v=20260916-equipped-source-1&subclass=20260916-hash-1&entry=20260916-equipped-1&navigation=20260919-1&champion=20260924-champion-export-1&plain=20260925-1';
import {revealRecommendedBuild,weaponCombinationsMarkup} from './recommended-build-reveal.mjs?v=20260916-weapon-combinations-1&plain=20260925-1';
import '../guardian-portal-progress.mjs?v=20260913-character-safe-2&loader=3&transport=20260911-compact-plugs-1&champion=20260924-champion-export-1&plain=20260925-1';
import '../guardian-vault-access.mjs?v=20260902-forge-loader-1';
import {bindParadoxItemInspect} from '../paradox-item-hover.mjs?v=20260913-presentation-consistency-1&status=20260917-compact-1&champion=20260924-champion-export-1&plain=20260925-1';
...
const forgePreparation=new ForgePreparationClient({onStatus:message=>{
  ...
  else if(status&&message.key===activePreparationKey)status.textContent=message.type==='ready'?'Build option ready':message.type==='error'?(message.message||'This option needs additional Bungie data.'):'Preparing build options…';
...
function renderArmourRecommendationState(build={}){
  ...
  if(state)state.textContent=generated?'PARADOX RECOMMENDATION · REVIEW REQUIRED':manual?'MANUAL WORKING BUILD':directEntryMode(build)?'ARMOUR & INSTALLED MODS':staged?'STAGED ARMOUR · MOD PLAN PENDING':'ARMOUR & INSTALLED MODS';
  if(instruction)instruction.textContent=generated?'AI mod plan generated · review the recommendation before live action':manual?'Undefined armour and reusable mods staged manually.':directEntryMode(build)?'Select an elemental build and generate from this equipment.':staged?'Choose armour manually, or select an elemental build and generate an AI sequence.':'Installed mods from this loaded build are shown below.';
...
async function startDirectGeneration(mode){
  ...
  }catch(error){recommendationFailure=error?.message||'Gear could not be prepared.';renderRecommendationControls(currentBuild()||{});}
...
function manualItemCard(item,index,selected){
  const name=esc(item?.name||'Destiny item'),exotic=Boolean(item?.isExotic||String(item?.tier||'').toLowerCase()==='exotic');
  return `<article class="manual-item-card${selected?' is-selected':''}${exotic?' is-exotic':''}"><span class="manual-item-inspect" data-manual-item-inspect="${index}" tabindex="0" title="${name}">${itemTileMarkup(item,{kind:manualEditorState.kind})}</span><button type="button" data-manual-item-index="${index}" aria-pressed="${selected}" aria-label="${selected?'Selected':'Select'} ${name}">${selected?'SELECTED':'SELECT'}</button></article>`;
}
function renderManualEditor(){
  ...
  byId('manualEditorItems').innerHTML=rows.length?rows.map((item,index)=>manualItemCard(item,index,manualItemId(item)===manualItemId(current))).join(''):'<div class="manual-editor-status is-bad">No item matches this slot and search.</div>';
  byId('manualEditorItems').querySelectorAll('[data-manual-item-inspect]').forEach(node=>bindParadoxItemInspect(node,rows[Number(node.dataset.manualItemInspect)],manualEditorState.kind));
  const status=byId('manualEditorStatus');if(status){status.className='manual-editor-status';status.textContent=manualInventory?`${rows.length} option${rows.length===1?'':'s'} · select an item, then choose any reusable socket option below.`:'Loading inventory…';}
  const groups=socketGroups(current,kind),optionMap=new Map();manualEditorState.socketOptions=optionMap;
  byId('manualEditorSockets').innerHTML=groups.length?`<div class="manual-socket-groups">${groups.map(group=>`<div class="manual-socket-group"><strong>${esc(group.label)} · SOCKET ${group.socketIndex+1}</strong><div class="manual-socket-options">${group.options.map((option,index)=>{const key=`${group.socketIndex}:${index}`,selected=manualHash(group.current)===manualHash(option),icon=abs(iconOf(option));optionMap.set(key,{socketIndex:group.socketIndex,option});return `<button type="button" class="manual-socket-option${selected?' is-selected':''}" data-manual-socket-option="${key}" aria-pressed="${selected}" title="${esc(option?.description||option?.name||'Socket option')}">${icon?`<img src="${esc(icon)}" alt="">`:'<span>◆</span>'}<b>${esc(option?.name||'Option')}</b></button>`;}).join('')}</div></div>`).join('')}</div>`:'<div class="manual-editor-status">No free, reversible socket alternatives are available for this exact item. Other changes remain available as explicit in-game steps.</div>';
}
async function openManualEditor(kind='weapon'){
  ...
  catch(error){const status=byId('manualEditorStatus');if(status){status.className='manual-editor-status is-bad';status.textContent=error?.message||'Undefined inventory could not be loaded.';}}
}
...
function createApplyReviewEntries(plan,state){
  ...
  for(const target of plan.equipment?.targets||[]){const item=applyEntity(index,{itemInstanceId:target.itemInstanceId,hash:target.itemHash});entries.push({key:`equipment:${target.itemInstanceId}`,type:'equipment',itemInstanceId:String(target.itemInstanceId),itemHash:Number(target.itemHash),name:String(target.name||item?.name||'Destiny item'),detail:`${String(target.kind||'item').toUpperCase()} · INSTANCE`,icon:abs(iconOf(item)),state:'ready',status:'READY TO EQUIP'});}
...
function updateApplyProgress(row={}){
  ...
  if(row.status==='running'){
    if(row.phase==='snapshot')executable.forEach(entry=>setApplyEntry(entry,'queued','CHECKING','Waiting for current Bungie data.'));
    ...
    else if(row.phase==='readback')executable.filter(entry=>!['failed','blocked'].includes(entry.state)).forEach(entry=>setApplyEntry(entry,'verifying','LOADING','Waiting for Bungie readback.'));
...
function finaliseApplyEntries(result){
  ...
  for(const entry of pendingApplyEntries){
    ...
    if(entry.type==='equipment'){
      ...
      if(attempted&&!failedEquipIds.has(entry.itemInstanceId)&&equippedIds.has(entry.itemInstanceId))setApplyEntry(entry,'success','CONFIRMED','Bungie readback confirms this exact item equipped.');
    ...
    if(completed&&!mismatch&&readback&&Array.isArray(readback.socketMismatches))setApplyEntry(entry,'success','CONFIRMED','Bungie readback confirms this exact socket value.');
...
function showApplyResult(result,error=null){
  ...
  applyDialogMode='result';if(kicker)kicker.textContent='APPLY RESULT';if(title)title.textContent=failures?(successes?'LOADOUT PARTIALLY APPLIED':'LOADOUT NOT APPLIED'):'LOADOUT SUCCEEDED';if(guardian)guardian.textContent=`${successes} change${successes===1?'':'s'} confirmed · ${failures} failed or unconfirmed${unsupported?` · ${unsupported} in-game only`:''}`;if(warning)warning.textContent=failures?'Only entries marked Confirmed were applied. Completed changes were not rolled back.':unsupported?'Remote changes were confirmed. Entries marked In Game Only were not sent to Bungie.':'All listed changes were applied.';
...
async function openApplyConfirmation(){
  ...
  const request=++livePreflightRequest;livePreflightBusy=true;renderApplyControls(currentBuild()||{},{preserveBanner:true});setApplyPreflightStatus('Checking this build before Apply. No changes have been made.','running');
  try{
...
async function executeConfirmedApply(){
  ...
  try{
    ...
    if(result.status==='applied')setLiveActionBanner(`Apply · Bungie readback matched all ${plan.equipment.targets.length} exact equipment targets${plan.socketChanges.length?` and ${plan.socketChanges.length} socket change${plan.socketChanges.length===1?'':'s'}`:''}.`,'good');
    else if(result.status==='blocked')setLiveActionBanner(`No live changes made · ${result.steps.find(row=>row.status==='blocked')?.detail?.[0]||'fresh inventory validation blocked Apply.'}`,'bad');
...
function unavailableCard(label='Details unavailable'){return '<button type="button" class="selector-card is-unavailable" disabled title="Bungie did not supply an option for this slot"><span class="selector-glyph">◇</span><small>'+esc(label)+'</small></button>';}
...
function artifactPerkCard(perk,index,{compact=false,selected=null,recommended=false,recommendation=null}={}){
  const icon=abs(iconOf(perk)),name=esc(perk?.name||'Artifact perk'),description=esc(perk?.description||perk?.definition?.displayProperties?.description||'Bungie Artifact perk');
...
function artifactRecommendationMarkup(build){
  ...
  if(!recommendation)return '<div class="artifact-recommendation is-neutral"><b>LOADING CURRENT ARTIFACT</b><small>Checking the active Bungie season and legal perk matrix.</small></div>';
  ...
  if(recommendation.selectionStatus!=='ready'){
    const blocker=esc(recommendation.blockers?.[0]||'A complete Artifact match is not available.');
    return '<div class="artifact-recommendation is-blocked"><b>BEST FIT NOT STAGED</b><small>'+blocker+' The live Guardian remains unchanged.</small><button type="button" data-artifact-recommend>CHECK ARTIFACT</button></div>';
  }
  ...
  const reasons=strongest.map(row=>'<li><b>'+esc(row.artifactPerk?.name||'Artifact perk')+'</b><span>'+esc(row.reasons?.[0]?.label||'Forge Loader compatibility')+'</span></li>').join('');
  const artifactChoice=recommendation.artifactCandidateCount>1?esc(`${recommendation.artifactName||'Artifact'} · best of ${recommendation.artifactCandidateCount} Artifacts`):esc(recommendation.artifactName||'Artifact');
  const fullTarget=recommendation.planMode==='full-build-target';
  const planLabel=fullTarget?'PARADOX FULL TARGET PLAN':'PARADOX BEST FIT';
  const planDetail=fullTarget
    ?'This complete target plan is ranked from the current Artifact perk tree even when Bungie reports no unused unlock points. Apply the picks in game as they become available. Working Build only · currently unlocked and equipped perks remain unchanged.'
    :'Artifact 2.0 is ranked from its socket buckets, then each bucket is filled from the staged Exotic, stat priorities, armour-set traits, subclass/Super and weapons. Working Build only · currently unlocked and equipped perks remain unchanged.';
...
function artifactMatrix(artifact,configuration,recommendation){
  ...
  if(!perks.length)return '<div class="artifact-spectrum-unavailable"><b>FULL PERK SPECTRUM UNRESOLVED</b><small>Requires Artifact socket-bucket or CharacterProgressions perk definitions from Bungie.</small></div>';
...
function stageSelection(kind,index){
  ...
  stageWorkingBuild(working=>{
    ...
    recordManualEdit(working,{component:kind,optionHash:Number(candidate.hash??candidate.itemHash??candidate.bungieHash)||null,optionName:String(candidate.name||candidate.displayName||'Option')});
...
function decisionReasonText(row={}){const reason=row.reasons?.[0];return reason?.label||`${row.componentName||'Component'} retained; no stronger alternative was found.`;}
...
function renderModRecommendation(build={}){
  ...
  if(!plan){host.innerHTML='<div class="review-mod-unavailable">No armour-mod plan was generated.</div>';return;}
  ...
  host.innerHTML=`<div class="review-mod-summary"><div><b>RAW → CURRENT → RECOMMENDED</b><span>Forge Loader raw stats remain mod-free. Installed and proposed mod contributions are evaluated separately.</span></div><div class="review-mod-stats">${statRows}</div><small>${changeSummary}</small></div><div class="review-mod-items">${itemRows||'<div class="review-mod-no-changes">No mod change improves this Working Build.</div>'}</div>${plan.limitations?.length?`<ul class="review-mod-limitations">${plan.limitations.slice(0,5).map(row=>`<li>${esc(row)}</li>`).join('')}</ul>`:''}`;
}
function renderForgeDecision(build={}){
  ...
  if(!build.forgeLoaderDecision){label.textContent='Choose armour';detail.textContent='Choose a direct entry or a Forge Loader result to begin armour-led build reasoning.';listNode.innerHTML='<li>Generate a build to see recommendations.</li>';return;}
  if(!decision){const anchor=build.forgeLoaderDecision?.buildAnchor?.perk?.name||build.forgeLoaderDecision?.buildAnchor?.name||'Exotic',setName=build.forgeLoaderDecision?.setProtocol?.[0]?.setName||build.forgeLoaderDecision?.setProtocol?.[0]?.trait?.name||'';label.textContent='ARMOUR DATA STAGED';detail.textContent='Generate Max Loadout to compare the subclass catalogue against the selected armour.';listNode.innerHTML=`<li>${esc(anchor)} anchors the recommendation.</li>${setName?`<li>${esc(setName)} contributes to the match score.</li>`:''}`;return;}
  const evidence=decision.evidence||{},coverage=decision.prismaticCoverage,rows=(decision.decisions||[]).filter(row=>row.score>0).slice(0,3),partial=build.forgeEvidence?.status==='partial',pending=build.forgeEvidence?.pending||[];label.textContent=partial?`${String(decision.element||'').toUpperCase()} PARTIAL BUILD READY FOR REVIEW`:`${String(decision.element||'').toUpperCase()} BUILD READY FOR REVIEW`;detail.textContent=partial?`${pending.length} unresolved data field${pending.length===1?' was':'s were'} excluded from scoring. The Working Build completed for review.`:`${Number(evidence.directedLinks||0)} directed data link${Number(evidence.directedLinks||0)===1?'':'s'} · ${decision.decisions?.length||0} subclass component decisions${coverage?` · ${coverage.covered.length}/5 Prismatic elements available`:''}.`;
  listNode.innerHTML=(partial?pending.slice(0,3).map(row=>`<li>${esc(row.message)}</li>`).join(''):'')||(rows.length?rows:(decision.decisions||[]).slice(0,3)).map(row=>`<li>${esc(decisionReasonText(row))}</li>`).join('')||(decision.limitations||[]).slice(0,2).map(row=>`<li>${esc(row)}</li>`).join('')||'<li>Keep the equipped build; no stronger alternative was found.</li>';
}
function renderRecommendedBuildReview(build={}){
  const subclass=resolvedSubclassOptions(build).find(item=>elementOf(item)===elementOf({element:build.subclass||build.subclassName||''})),superItem=Object.hasOwn(build.subclassBuild||{},'super')?build.subclassBuild.super:build.super,abilities=build.subclassBuild?.abilities||[],aspects=build.subclassBuild?.aspects||[],fragments=build.subclassBuild?.fragments||[],anchor=build.forgeLoaderDecision?.buildAnchor||{},anchorName=anchor.name||'EXOTIC',anchorPerk=anchor.perk||null,exoticRule=validateExoticLoadout(build,{requireArmourAnchor:true}),partial=build.forgeEvidence?.status==='partial',activity=forgeActivityOption(build.activityContext);
  byId('recommendedBuildTitle').textContent=partial?'PARTIAL WORKING BUILD':'RECOMMENDED BUILD';
  byId('recommendedBuildSubtitle').textContent=`${String(build.characterClass||'Guardian').toUpperCase()} · ${String(build.recommendationElement||build.subclassName||build.subclass||'subclass').toUpperCase()} · ${String(build.objective||'balanced').toUpperCase()} · ${activity?.label||'ACTIVITY UNRESOLVED'} · EXOTIC ANCHOR: ${String(anchorName).toUpperCase()}`;
  const safety=byId('recommendedBuildSafety');if(safety){safety.classList.toggle('is-partial',partial);safety.querySelector('b').textContent=partial?'PARTIAL DATA · LIVE GUARDIAN UNCHANGED':'LIVE GUARDIAN UNCHANGED';safety.querySelector('span').textContent=partial?build.forgeEvidence.statement:'This recommendation exists only in the protected Working Build until you explicitly confirm Apply.';}
  const subclassHost=byId('recommendedSubclassSummary');
  subclassHost.innerHTML=`<section class="subclass-super-shell equipped-subclass-stack"><div class="equipped-subclass" aria-label="Working Build subclass"><span class="equipped-subclass__crest"><img alt="" hidden></span><span class="equipped-subclass__copy"><strong></strong><small></small></span></div><section class="super-feature" aria-label="Working Build Super and compatible alternatives"><div class="super-feature__cluster"><div class="super-diamond super-diamond--equipped" data-super-slot="equipped"><span>◆</span></div>${[1,2,3,4,5].map(index=>`<div class="super-diamond super-diamond--alt super-diamond--alt${index}" data-super-slot="alternate-${index}"><span>◆</span></div>`).join('')}</div><div class="super-feature__name"></div></section></section><p class="review-super-baseline" hidden></p><div class="review-socket-group"><b>ABILITIES</b><div class="build-equipped-slots build-ability-slots">${abilities.map(tile).join('')||'<small>NO ABILITIES</small>'}</div></div><div class="review-socket-group"><b>ASPECTS</b><div class="build-equipped-slots build-aspect-slots">${aspects.map(tile).join('')||'<small>NO ASPECTS</small>'}</div></div><div class="review-socket-group"><b>FRAGMENTS</b><div class="build-equipped-slots build-fragment-slots">${fragments.map(tile).join('')||'<small>NO FRAGMENTS</small>'}</div></div>`;
  const identity=subclassHost.querySelector('.equipped-subclass');
  renderEquippedSubclass({root:identity,iconNode:identity.querySelector('img'),nameNode:identity.querySelector('strong'),metaNode:identity.querySelector('small'),subclass:build.subclass||'',subclassName:build.subclassName||'',characterClass:build.characterClass||'Guardian',icon:iconOf(subclass)||build.subclassIcon||''});
  renderSuperFormation({host:subclassHost.querySelector('.super-feature__cluster'),nameNode:subclassHost.querySelector('.super-feature__name'),activeSuper:superItem,superOptions:build.subclassBuild?.superOptions||[],subclass:build.subclass||build.subclassName||'',subclassCatalog:resolvedSubclassOptions(build),characterClass:build.characterClass||'Guardian'});
  // The review displays the Working Build without turning compatible choices into live edits.
  subclassHost.querySelectorAll('[data-super-slot]').forEach(node=>{node.tabIndex=-1;node.removeAttribute('role');});
  const original=readState()?.originalBuild,originalSuper=Object.hasOwn(original?.subclassBuild||{},'super')?original.subclassBuild.super:original?.super,baseline=subclassHost.querySelector('.review-super-baseline');
  if(originalSuper&&String(originalSuper.hash??originalSuper.itemHash)!==String(superItem?.hash??superItem?.itemHash)){baseline.hidden=false;baseline.textContent=`Starting loadout Super: ${originalSuper.name||originalSuper.displayName||'Super'}. The recommendation above changes only the Working Build.`;}
  const intelligenceHost=byId('recommendedIntelligenceSummary'),decisions=build.forgeIntelligence?.decisions||[],matched=decisions.filter(row=>row.score>0).slice(0,4),reviewRows=matched.length?matched:decisions.slice(0,4),anchorDescription=String(anchorPerk?.description||'').trim(),anchorReason=anchorDescription?`EXOTIC ANCHOR · ${anchorName}${anchorPerk?.name?` · ${anchorPerk.name}`:''} contributes only its effect text to the ranking.`:`EXOTIC ANCHOR · ${anchorName} effect details are unavailable.`,pendingRows=(build.forgeEvidence?.pending||[]).map(row=>`<li>${esc(row.message)}</li>`).join(''),superLimitations=(build.forgeIntelligence?.superSynergy?.limitations||[]).map(row=>`<li>${esc(row)}</li>`).join('');if(intelligenceHost)intelligenceHost.innerHTML=`<li>${esc(anchorReason)}</li>`+pendingRows+superLimitations+(reviewRows.map(row=>`<li>${esc(decisionReasonText(row))}</li>`).join('')||(build.forgeIntelligence?.limitations||[]).map(row=>`<li>${esc(row)}</li>`).join('')||'<li>No additional recommendations.</li>');
  const armourHost=byId('recommendedArmourSummary')?.querySelector('.gear-columns');if(armourHost){armourHost.innerHTML=Array.from({length:5},(_,index)=>armourCard(index,build.armour?.[index])).join('');armourHost.querySelectorAll('.gear-slot .arm').forEach((node,index)=>bindParadoxItemInspect(node,build.armour?.[index],'armour'));}
  ...
  if(weaponRule)weaponRule.textContent=`VAULT + CHARACTER INVENTORY · ${Number(weaponConstraint.selectedExoticWeaponCount||0)}/1 EXOTIC WEAPON`;
  const weaponReviewHost=byId('recommendedWeaponsSummary');weaponReviewHost.innerHTML=Array.from({length:3},(_,index)=>{
    ...
    return `<article class="review-weapon paradox-model-card" data-review-weapon="${index}" data-weapon-tier="${Number.isInteger(tier)?tier:''}"><div><span class="review-item-inspect" tabindex="0" title="${esc(item.name||`Weapon ${index+1}`)}">${itemTileMarkup(item,{kind:'weapon'})}</span><span><b>${esc(item.name||`Weapon ${index+1}`)}</b><small>${esc(item.itemTypeDisplayName||item.weaponType||'Weapon')}</small><em>${esc(decisionLabel)} · ${Number(selection?.candidateCount||0)} CANDIDATES</em></span></div><div class="review-weapon-tier-model"><b>${Number.isInteger(tier)?`TIER ${tier}`:'TIER UNRESOLVED'} · ${rowCount} PERK ROW${rowCount===1?'':'S'}</b>${perkMatrix||'<small>PERK MODEL UNAVAILABLE</small>'}${traitHierarchy}</div><p>${esc(selection?.reasons?.[0]?.label||'Item retained; no stronger synergy was found.')}</p></article>`;
  }).join('');
  // Use the actual shared renderer's column count, including fallback perk models.
  const perkColumns=[...weaponReviewHost.querySelectorAll('.weapon-perk-matrix')].map(node=>Number(node.style.getPropertyValue('--weapon-perk-columns'))||1);
  weaponReviewHost.style.setProperty('--review-perk-columns',String(Math.max(1,...perkColumns)));
...
function renderBuildSurface(){
  ...
  if(!build||!original){
    ...
    byId('sourceLabel').textContent=recovering?'RECOVERING GUARDIAN':'NO BUILD SNAPSHOT FOUND';byId('sourceDetail').textContent=recovering?'Rebuilding the protected Original Build before the staged armour is applied.':'Return to the Guardian page, load a Guardian or Bungie loadout, then press Improve My Guardian.';
    byId('buildStateLabel').textContent=recovering?'PROTECTING ORIGINAL BUILD':'SNAPSHOT UNAVAILABLE';byId('buildStateDetail').textContent=recovering?'The Working Build will remain separate from the equipped Bungie baseline.':'No Original or Working Build has been loaded.';
    byId('artifactStatus').textContent='NOT CHECKED';byId('artifactStatusDetail').textContent='Artifact resolution starts only after a build snapshot is loaded.';
  ...
  const changes=diffBuilds(original,build),loadoutNumber=Number.isInteger(build.selectedLoadoutIndex)?build.selectedLoadoutIndex+1:null,sourceName=loadoutNumber?(build.loadoutSource==='currently-equipped'?`CURRENT EQUIPPED GUARDIAN · MATCHES SLOT ${loadoutNumber}`:`BUNGIE LOADOUT ${loadoutNumber}`):'CURRENT EQUIPPED GUARDIAN';
