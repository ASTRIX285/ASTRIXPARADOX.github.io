...
const forgePreparation=new ForgePreparationClient({onStatus:message=>{
  ...
  else if(status&&message.key===activePreparationKey)status.textContent=message.type==='ready'?'Build option ready':message.type==='error'?(message.message||'This option needs additional verified data.'):'Preparing build options…';
...
function renderRecommendedBuildReview(build={}){
  const subclass=resolvedSubclassOptions(build).find(item=>elementOf(item)===elementOf({element:build.subclass||build.subclassName||''})),superItem=build.subclassBuild?.super||build.super,abilities=build.subclassBuild?.abilities||[],aspects=build.subclassBuild?.aspects||[],fragments=build.subclassBuild?.fragments||[],anchor=build.forgeLoaderDecision?.buildAnchor||{},anchorName=anchor.name||'VERIFIED EXOTIC',anchorPerk=anchor.perk||null,exoticRule=validateExoticLoadout(build,{requireArmourAnchor:true}),partial=build.forgeEvidence?.status==='partial',activity=forgeActivityOption(build.activityContext);
  ...
  const weaponReviewHost=byId('recommendedWeaponsSummary');weaponReviewHost.innerHTML=Array.from({length:3},(_,index)=>{
    ...
    return `<article class="review-weapon paradox-model-card" data-review-weapon="${index}" data-weapon-tier="${Number.isInteger(tier)?tier:''}" tabindex="0" role="button"><div>${reviewIcon(item,'Weapon')}<span><b>${esc(item.name||`Weapon ${index+1}`)}</b><small>${esc(item.itemTypeDisplayName||item.weaponType||'Verified owned weapon')}</small><em>${esc(decisionLabel)} · ${Number(selection?.candidateCount||0)} OWNED CANDIDATES</em></span></div><div class="review-weapon-tier-model"><b>${Number.isInteger(tier)?`TIER ${tier}`:'TIER UNRESOLVED'} · ${rowCount} PERK ROW${rowCount===1?'':'S'}</b>${perkMatrix||'<small>NO VERIFIED PERK MODEL</small>'}${traitHierarchy}</div><p>${esc(selection?.reasons?.[0]?.label||'Exact owned instance retained; no stronger explicit synergy evidence was proven.')}</p></article>`;
  }).join('');
