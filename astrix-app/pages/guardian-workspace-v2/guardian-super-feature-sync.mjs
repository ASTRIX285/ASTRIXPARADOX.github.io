...
function populateSuperChain(detail={}){const host=document.getElementById('superFeatureCluster');if(!host)return;const characterClass=String(detail.characterClass||'hunter').toLowerCase();const subclass=String(detail.subclass||'').trim().toLowerCase();const catalog=mergeSubclassCatalog(detail.subclassCatalog,characterClass);const catalogEntry=catalog.find(item=>elementKey(item)===subclass)||null;const suppliedBuild=detail?.subclassBuild||{};const baseBuild=catalogEntry?.subclassBuild||{};const options=mergeSuperOptions(characterClass,subclass,[...(Array.isArray(baseBuild.superOptions)?baseBuild.superOptions:[]),...(Array.isArray(suppliedBuild.superOptions)?suppliedBuild.superOptions:[]),detail.super,suppliedBuild.super].filter(Boolean));const requested=Object.hasOwn(suppliedBuild,'super')?suppliedBuild.super:detail.super||null;const active=options.find(item=>String(item?.hash??item?.itemHash??'')===String(requested?.hash??requested?.itemHash??''))||null;renderSuperFormation({host,nameNode:document.getElementById('subclassName'),activeSuper:active,superOptions:options,subclass,subclassCatalog:catalog,characterClass,onSelect:item=>{const nextBuild={...baseBuild,...suppliedBuild,super:item,superOptions:options};const nextCatalog=catalog.map(entry=>elementKey(entry)===subclass?{...entry,subclassBuild:nextBuild}:entry);dispatchGuardianSelection({...detail,loadoutSource:'subclass-preview',selectedLoadoutIndex:null,subclassCatalog:nextCatalog,subclassBuild:nextBuild,super:item,superOptions:options});}});syncTranscendence(host,detail,subclass==='prismatic');const label=document.getElementById('subclassName');if(label)label.dataset.superName=active?.name||'';}
