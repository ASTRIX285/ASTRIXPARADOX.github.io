import {setBonusOptions} from './forge-loader-model.mjs?v=20260904-top-50-scan-1&plain=20260925-1';
...
export function forgeSetListMarkup(options){
  if(!options.length)return '<div class="forge-empty">No armour set definitions are available.</div>';
  ...
  const cards=options.map(row=>{
    ...
    const state=`${row.owned?'In inventory':'Not in inventory'} · ${row.usableSlots} compatible slots with this Exotic`;
    return `<article class="forge-set" data-forge-set="${row.hash}"><button type="button" class="forge-set-head" ${perkTooltipAttributes({name:row.name,description:details,itemTypeDisplayName:'Armour set'},state)}><span class="forge-set-icon${row.owned?' is-owned':''}">${row.icon?`<img src="${esc(row.icon)}" alt="">`:''}</span><strong>${esc(row.name)}</strong></button><div class="forge-set-choices">${[2,4].map(count=>{
      ...
      const reason=choice.checked?'Selected':choice.disabled?'Unavailable with current pieces and selected set requirements':'Available with pieces';
