...
export function forgeInventorySignature(payload){
  ...
  return JSON.stringify([payload?.pageReady?.manifestVersion||payload?.manifestVersion,payload?.pageReady?.coverage,payload?.definitionCoverage,payload?.artifactCoverage,payload?.forgeArmourIndex?.generatedAt,profile.profileInventory,profile.characterInventories,profile.characterEquipment,profile.characterLoadouts,profile.itemComponents?.instances,profile.itemComponents?.stats,profile.itemComponents?.sockets]);
}
