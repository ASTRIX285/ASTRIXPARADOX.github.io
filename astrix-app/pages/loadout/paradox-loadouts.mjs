...
function savedMods(item){
  ...
  return mods.length?mods:(item.mods||[]).filter(Boolean);
}
...
function renderDetail(){
  ...
  host.innerHTML=`<div class="paradox-loadout-detail-head"><div><small>PARADOX SAVED BUILD · REVISION ${Number(record.revision||1)}</small><h2>${esc(record.name)}</h2></div><div class="paradox-loadout-detail-actions"><button type="button" data-saved-action="open">OPEN IN BUILD FORGE</button><button type="button" data-saved-action="download">DOWNLOAD JSON</button><button type="button" class="is-danger" data-saved-action="delete">DELETE</button></div></div>${record.description?`<p class="paradox-loadout-description">${esc(record.description)}</p>`:''}${savedBuildOverview(build)}${manualSteps.length?component('IN-GAME STEPS',manualSteps):''}<details class="paradox-loadout-provenance"><summary>Build details</summary>${component('PROVENANCE',[`${record.summary?.manualEditCount||0} manual edit${record.summary?.manualEditCount===1?'':'s'}`,record.source?.bungieLoadoutIndex==null?'Current equipped source':`Copied from Bungie slot ${record.source.bungieLoadoutIndex+1}`])}<div class="paradox-loadout-binding">GUARDIAN ${esc(record.binding.characterId)} · MEMBERSHIP ${esc(record.binding.membershipType)}:${esc(record.binding.membershipId)} · UPDATED ${esc(new Date(record.updatedAt).toLocaleString())}</div></details>`;
}
function render(){
